...
- Architecture note: `docs/architecture.md`
  - Includes the database schema generated by SQL migrations (tables, relationships, indexes, and triggers)
  - Includes demo seed data created by `V5__seed_demo_hierarchy.sql`
...
Run integration test with explicit test profile:

```bash
./mvnw "-Dspring.profiles.active=test" "-Dtest=OrganizationTreePersistenceIT" test
```

For full cross-platform setup and troubleshooting, see `docs/local-setup.md`.
